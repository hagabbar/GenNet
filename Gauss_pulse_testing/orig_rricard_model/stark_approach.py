...
def get_generative(G_in, dense_dim=128, drate=0.1, out_dim=50, lr=1e-3):
    ...
    x = Conv2DTranspose(128,(1,4),strides=(1,1),padding='same',activation='relu')(x)
    x = BatchNormalization()(x)
    #x = GaussianDropout(drate)(x)
    x = Conv2DTranspose(64,(1,8),strides=(1,1),padding='same',activation='relu')(x)
    x = BatchNormalization()(x)
    #x = GaussianDropout(drate)(x)
    x = Conv2DTranspose(32,(1,16),strides=(1,1),padding='same',activation='relu')(x)
    x = BatchNormalization()(x)
    #x = GaussianDropout(drate)(x)
    x = Conv2DTranspose(16,(1,32),strides=(1,1),padding='same',activation='relu')(x)
    x = BatchNormalization()(x)
    #x = GaussianDropout(drate)(x)
    x = BatchNormalization()(x)
    x = Flatten()(x)
...
def sample_data_and_gen(G, xt_train, noise_dim=10, n_samples=10000, noise_samples=100):
    # XT should be a bunch of random noise-free signals
    XT = sample_data(n_samples)
    # XN_noise should be the raw ht with signal burried in it.
    #XN_noise = np.resize(xt_train, (1,1,xt_train.shape[1]))
    XN_noise = []
    for sig in range(noise_samples):
        XN_noise.append(XT[sig] + np.random.normal(-hyperparams.noise_level, hyperparams.noise_level, size=[1, xt_train.shape[1]]))
    XN_noise = np.resize(XN_noise, (noise_samples,1,xt_train.shape[1]))
...
def sample_noise(G, xt_train, noise_dim=10, n_samples=10000):
    # pull a bunch of the target to-be-cleaned waveform
    # I think this is what I feed into the generator
    X = []
    for i in range(n_samples):
        X.append(sample_data(n_samples) + np.random.normal(-hyperparams.noise_level, hyperparams.noise_level, size=[1, xt_train.shape[1]]))
    X = np.resize(X, (n_samples,1,xt_train.shape[1]))
...
def main():
    ...
    ax = ax.get_figure()
    ax.savefig('%sinput_waveform.png' % outdir, dpi=1000)
    plt.close(ax)

    ax = pd.DataFrame(np.transpose(xt_train)).plot(legend=False)
    ax = ax.get_figure()
    ax.savefig('%sinput_plusnoise_waveform.png' % outdir, dpi=1000)
    plt.close(ax)
    ...
    ax.set_yscale("log")
    ax = ax.get_figure()
    ax.savefig('%sloss.png' % outdir, dpi=1000)
    plt.close(ax)
    ...
    ax = pd.DataFrame(np.transpose(data_and_gen[N_VIEWED_SAMPLES:])).plot(legend=False)
    ax = ax.get_figure()
    ax.savefig('%sgen_waveform.png' % outdir, dpi=1000)
    plt.close(ax)
